...
(defn write-batch
  ([{:keys [onyx.core/messenger onyx.core/state
   ...
   (write-batch event peer-replica-view state messenger (:egress-ids serialized-task)))
  ([event peer-replica-view state messenger egress-tasks]
...
(defrecord Function [peer-replica-view state messenger egress-tasks]
  ...
  (write-batch
    [_ event]
    (write-batch event peer-replica-view state messenger egress-tasks))
